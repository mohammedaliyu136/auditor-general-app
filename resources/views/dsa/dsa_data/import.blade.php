<x-layouts.app>
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">{{ __('Import DSA Data') }}</h1>
        <a href="{{ route('dsa-data.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white font-bold py-2 px-4 rounded">
            {{ __('Back to List') }}
        </a>
    </div>
    ...
    <div class="max-w-4xl mx-auto bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
        ...
        <div class="mb-8">
            ...
            <p class="text-gray-600 dark:text-gray-300 mb-4">
                {{ __('Please upload an Excel (.xlsx) or CSV file containing your DSA Data. To ensure a successful import, your file must match the required format.') }}
            </p>
            ...
            <a href="{{ route('dsa-data.download-sample') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 active:bg-indigo-900 focus:outline-none focus:border-indigo-900 focus:ring ring-indigo-300 disabled:opacity-25 transition ease-in-out duration-150">
        ...
        <div>
            ...
            <form action="{{ route('dsa-data.import') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="mb-4">
